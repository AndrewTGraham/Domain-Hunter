"""
Domain Hunter
Read the file README for a better understanding of this code and more
notes.
Written by Andrew Graham
Originally written Feb 11 2023, but later modified and uploaded to
GitHub Jun 1-2 2023.
"""
...
"""
To make pronounceable words, we will overlap two existing words making
sure they have at least 3 letters of overlap and ensuring that the start
of one word is the start of our generated word and the end of the other
word is the end of our generated word. For example, sURName and bURNt
can make surnt. Create two dataframes (word_starts and word_ends__)
which contain the start and end of words where the end of the start and
the start of the end are the overlap portions. The column start_len in
word_starts is the length of letters, but in word_ends__ is the length
of the word_starts letters needed to create a word of domain_length
(foreign key). word_ends__ letters column does not have the overlap
letters so I can join them easily later. Note: If I was regularly using
this, I would store the output of this cell in a separate file so I
don't have to generate this each time, but seeing as I'm only running
this once per domain length, that's unnecessary for me, but may be
useful for others. There are also opportunities to improve the
efficiency here, but for a one-time run, it's more efficient to not
optimize the code.
"""
word_starts = pd.DataFrame(columns=['a_letters',
                                    'a_count',
                                    'overlap',
                                    'start_len'])
word_ends__ = pd.DataFrame(columns=['b_letters',
                                    'b_count',
                                    'overlap',
                                    'start_len'])
...
"""
Some sets of letters will appear multiple times from having been in 
multiple words; I need to combine them.
"""
